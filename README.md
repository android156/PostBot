...
- **Асинхронная обработка** весовых категорий одного маршрута
...
4. **Асинхронный расчет стоимости** для всех весовых категорий
5. Поиск лучших предложений для каждого веса
6. Применение фильтров доставки
7. Генерация Excel файла с результатами
8. Отправка результатов пользователю
...
- `_calculate_route_all_weights(route)` - асинхронный расчет всех весов
- `_create_weight_calculation_tasks(route)` - создание задач для весов
- `_wait_for_rate_limiting()` - обработка rate limiting
...
3. Для каждого маршрута **асинхронно** рассчитывается стоимость доставки по 7 весовым категориям
4. Применяется фильтр типов доставки (по умолчанию "До дверей")
5. Находятся самые выгодные предложения для каждого веса
6. Создается Excel файл с результатами и отправляется пользователю
...
- **Параллельные запросы** для всех весов одного маршрута
- Улучшенная производительность в 3-5 раз
- Сохранение rate limiting между маршрутами
...
### Производительность
- **Асинхронные HTTP запросы** к API для каждого маршрута
- **Параллельная обработка** весовых категорий
